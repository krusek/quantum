...
namespace Microsoft.Quantum.Samples
{
    ...
    class Program
    {
        static async Task Main()
        {
            ...
            // data file.
            var data = await LoadData("training_data.json");
            ...
            var (optimizedParameters, optimizedBias) = await TrainHalfMoonModel.Run(
                targetMachine,
                new QArray<QArray<double>>(data.Features.Select(vector => new QArray<double>(vector))),
                new QArray<long>(data.Labels),
            ...
            var missRate = 0.0;
            // await ValidateHalfMoonModel.Run(
            //     targetMachine,
        ...
        static async Task<LabeledData> LoadData(string dataPath)
        {
            using var dataReader = File.OpenRead(dataPath);
            return await JsonSerializer.DeserializeAsync<LabeledData>(
                dataReader
            );
        }
